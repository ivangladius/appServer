...
import appServer2.MessageObject;

public class Server2 {

    private int port;

    private Server2Socket serverSocket;

    private ThreadPoolExecutor executor;

    public Server2(int port, int threadPoolSize) {
    ...
    public void start() {
        try {
            serverSocket = new Server2Socket(port);
        } catch(IOException e) {
            stop();
            e.printStackTrace();
        }
...
class MyRunnable implements Runnable {
    ...
    public MyRunnable(Server2 server, Socket client) {
        this.server = server;
        this.client = client;
    }
